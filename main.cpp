...
public:
    ...
    pointer allocate(size_type n) { // Метод выделения памяти
        if (n > BlockSize) { // Если запрашиваемое количество больше размера блока
            return static_cast<pointer>(::operator new(n * sizeof(T))); // Используем стандартный оператор new
        }
        if (free_blocks.empty()) { // Если нет свободных блоков
    ...
    void deallocate(pointer p, size_type n) noexcept { // Метод освобождения памяти
        if (n > BlockSize) { // Если освобождаем больше блока
    ...
    void construct(U* p, Args&&... args) { // Метод конструирования объекта в выделенной памяти
        new (p) U(std::forward<Args>(args)...); // Вызываем конструктор объекта U на адресе p с аргументами args
    }

    template <typename U>
    void destroy(U* p) { // Метод разрушения объекта в выделенной памяти
        p->~U(); // Вызываем деструктор объекта U по адресу p
    }

private:
    void expand() { // Метод увеличения количества свободных блоков
        for (size_t i = 0; i < BlockSize; ++i) { // Для каждого блока в размере BlockSize
...
public:
    ...
    void push_back(const T& value) { // Метод добавления элемента в конец контейнера
    ...
    void print() const { // Метод вывода всех элементов контейнера на экран
        for (const auto& ptr : elements_) { // Для каждого указателя в векторе элементов 
    ...
    ~MyContainer() {  // Деструктор контейнера 
        for (auto ptr : elements_) {  // Для каждого указателя в векторе элементов 
